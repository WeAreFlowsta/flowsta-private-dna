...
pub fn get_user_profile(_: ()) -> ExternResult<Option<Record>> {
    ...
    if let Some(link) = links.first() {
        ...
        let details = get_details(profile_hash, GetOptions::default())?
            .ok_or(wasm_error!("Profile not found"))?;
        
        // Follow the update chain to get the latest record
        match details {
            Details::Record(record_details) => {
                // If there are updates, get the latest one
